...
const FAQS = [
  ...
  { q: 'How does the Travel Style feature work?', a: 'Select your destination country, state, city, travel month, and occasion. MAK considers the local weather, culture, and event type to suggest outfits and makeup looks.' },
...
export default function ProfileScreen() {
  ...
  const handleShareApp = async () => {
    try {
      ...
      if (error.message !== 'User did not share') Alert.alert('Oops!', 'Unable to share right now.');
    }
  };
  ...
  const submitFeedback = async () => {
    ...
    try {
      ...
      setShowFeedback(false); setFeedbackComment(''); setFeedbackRating(5);
    } catch (err: any) {
  ...
  const menuItems = [
    ...
    { icon: 'shield-checkmark-outline', title: 'Privacy Policy', description: 'Your data is safe', onPress: () => Alert.alert('Privacy Policy', 'MAK does not store any personal identifying information. Your photos are analyzed in real-time and immediately discarded.'), accent: colors.tertiary },
  ];
    ...
    <SafeAreaView style={[st.container, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={st.scroll} showsVerticalScrollIndicator={false}>
        <View style={st.headerRow}>
          <TouchableOpacity onPress={() => router.push('/(tabs)')} style={st.backBtn}>
        ...
        {!showLogoutConfirm ? (
          <TouchableOpacity style={[st.logoutBtn, { borderColor: colors.error + '40' }]} onPress={() => setShowLogoutConfirm(true)}>
            <Ionicons name="log-out-outline" size={22} color={colors.error} />
            <Text style={[st.logoutText, { color: colors.error }]}>Logout</Text>
          </TouchableOpacity>
        ) : (
          <View style={[st.logoutConfirm, { backgroundColor: colors.error + '0D', borderColor: colors.error + '30' }]}>
            <Text style={[st.logoutConfirmText, { color: colors.text }]}>Are you sure you want to logout?</Text>
            <View style={st.logoutConfirmBtns}>
              <TouchableOpacity style={[st.logoutCancelBtn, { borderColor: colors.border }]} onPress={() => setShowLogoutConfirm(false)}>
                <Text style={[st.logoutCancelText, { color: colors.textSecondary }]}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[st.logoutYesBtn, { backgroundColor: colors.error }]} onPress={handleLogout} disabled={loggingOut}>
                {loggingOut ? <ActivityIndicator color="#FFF" size="small" /> : <Text style={st.logoutYesText}>Yes, Logout</Text>}
              </TouchableOpacity>
            </View>
          </View>
        )}
        ...
        {showFeedback && (
          <View style={[st.overlay, { backgroundColor: colors.overlay }]}>
            <View style={[st.modal, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              ...
              <View style={st.stars}>{[1,2,3,4,5].map(s => (<TouchableOpacity key={s} onPress={() => setFeedbackRating(s)}><Ionicons name={s <= feedbackRating ? 'star' : 'star-outline'} size={30} color={colors.primary} /></TouchableOpacity>))}</View>
              <Text style={[st.label, { color: colors.textSecondary }]}>Category</Text>
              <View style={st.catOptions}>{categories.map(c => (<TouchableOpacity key={c.id} style={[st.catOption, { backgroundColor: feedbackCategory === c.id ? colors.primary : colors.surfaceVariant, borderColor: feedbackCategory === c.id ? colors.primary : colors.border }]} onPress={() => setFeedbackCategory(c.id)}><Ionicons name={c.icon as any} size={16} color={feedbackCategory === c.id ? '#FFF' : colors.primary} /><Text style={[st.catText, { color: feedbackCategory === c.id ? '#FFF' : colors.primary }]}>{c.label}</Text></TouchableOpacity>))}</View>
...
const st = StyleSheet.create({
  ...
  logoutConfirm: { borderRadius: 14, padding: 18, borderWidth: 1, marginBottom: 16 },
  logoutConfirmText: { fontSize: 15, fontWeight: '600', textAlign: 'center', marginBottom: 14 },
  logoutConfirmBtns: { flexDirection: 'row', gap: 12 },
  logoutCancelBtn: { flex: 1, padding: 14, borderRadius: 10, borderWidth: 1, alignItems: 'center' },
  logoutCancelText: { fontSize: 14, fontWeight: '600' },
  logoutYesBtn: { flex: 1, padding: 14, borderRadius: 10, alignItems: 'center' },
  logoutYesText: { color: '#FFF', fontSize: 14, fontWeight: '700' },
